...
#     labels = torch.from_numpy(labels.argmax(axis=1)).long()
#
...
def load_data(dataset):
    base_path = os.path.join("/home/myx/pythonfiles/myx/GNN+LPA/Unifying Graph Convolutional Neural Networks and Label Propagation/GCN-LPA-PyTorch/data", dataset)
    names = ['x', 'y', 'tx', 'ty', 'allx', 'ally', 'graph']
    objects = []
    for i in range(len(names)):
        with open('{}/ind.{}.{}'.format(base_path, dataset, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
    ...
    x, y, tx, ty, allx, ally, graph = tuple(objects)
    test_idx_reorder = parse_index_file("{}/ind.{}.test.index".format(base_path, dataset))
    test_idx_range = np.sort(test_idx_reorder)
